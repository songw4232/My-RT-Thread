...
`addPerson(Addressbooks  abs)`写成这样对吗？这是值传递！！它不能改变这个结构体变量的值，那这个函数传入的参数是什么，是地址！！正解：`addPerson(Addressbooks *abs)`
...
```C++
//1、添加联系人信息
void addPerson(Addressbooks *abs)//利用地址传递，可以修饰实参！！
{
	//判断电话本是否满了，先判断！！
	if (abs->m_Size == MAX)
	{
...
>  解析：`abs->personArray[abs->m_Size].m_Name = name;`重点这代码！！如何写，如何赋值！！
>
> `struct Addressbooks abs`:定义通讯录结构体变量；`struct Person personArray[MAX]`;://通讯录中保存的联系人数组，定义联系人结构体变量。
>
...
`(abs->personArray[i].m_Sex == 1 ? "男" : "女")`：这个可以学一下，三目运算符！！方便！！
...
`abs->personArray[ret].m_Sex = sex;`注意，需要修改的值是哪一个！！
...
**重点每个功能的逻辑，是如何实现的，知道逻辑，才知道代码怎么写！！**
